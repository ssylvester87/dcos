...
local function request(url, accept_404_reply)
    ...
    if SERVICE_AUTH_TOKEN ~= nil then
        headers = {["Authorization"] = "token=" .. SERVICE_AUTH_TOKEN}
    end
...
local function fetch_and_store_marathon_leader()
    ...
    -- Expected to run within lock context.
    local mleaderRes, err = request(UPSTREAM_MARATHON .. "/v2/leader", true)
...
local function fetch_and_store_state_mesos()
    ...
    -- Expected to run within lock context.
    local mesosRes, err = request(UPSTREAM_MESOS .. "/master/state-summary", false)
